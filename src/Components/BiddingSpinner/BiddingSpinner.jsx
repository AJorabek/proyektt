...
function BiddingSpinner(props) {
  ...
  const switchMultiplier = (currentIndex) => {
    ...
    const currentMultiplierIndex = updatedMultipliers.findIndex(
      (item) => item.current
    );
  ...
  const handleClick = () => {
    ...
    props.handleProfit(
      parseFloat(
        multipliers[currentMultiplierIndex].multiplier * props.bidAmount
      ).toFixed(2)
    );
    const nextMultiplierIndex =
      (currentMultiplierIndex - 1) % multipliers.length;
          ...
          <div ref={cardRef} class="   custom-table w-100 ">
            ...
            {multipliers.map((multi, i) => {
              return (
                <div
                  key={i}
                  className=" button-container d-flex flex-row align-content-around justify-content-start m-auto"
                >
                  <div class="multiplier-container  ">
                    <div class=" multiplier">x {multi.multiplier}</div>
                  </div>

                  <div class="apples-column-container me-2">
                    <ArrowButton
                      isActvated={multi.current}
