  ...
  return (
    <div class="bid-wrapper d-flex flex-column fixed-bottom  w-100 ">
      <div className="row flex-row-reverse mb-4 justify-content-around">
        <button className=" btn  mt-0  rounded custombtn">farthest</button>
        <button className=" btn  mt-0  rounded custombtn "> X2</button>
        <button className=" btn  mt-0 rounded custombtn"> X/2</button>
        <button className=" btn  mt-0 rounded custombtn">lowest</button>
      </div>
      <form className="d-flex flex-column" onSubmit={handleSubmit}>
        <input
          onChange={watchChange}
          min={0}
          max={localStorage.getItem("money")}
          className="  bid-amount mt-2"
          id="bid"
          type="number"
          step="any"
          placeholder=" "
          required
        ></input>
        <label class="bid-label" for="bid">
          bet amount
        </label>
        <button className=" btn bid-btn mt-0 rounded ">bet</button>
      </form>
    </div>
  );
}
